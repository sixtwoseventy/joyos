...
uint8_t
confdb_item_exists(uint16_t id) {
  return false;
}

uint8_t
confdb_load_item(uint16_t id, ConfdbItem *item) {
...
uint8_t
confdb_save_item(ConfdbItem *item) {
...
uint8_t 
confdb_load_integer(uint16_t id, uint16_t *value) {
    ConfdbItem item;
...
uint8_t 
confdb_save_integer(uint16_t id, uint16_t value) {
    ConfdbItem item;
    confdb_load_item(id, &item);
    if (item.type!=CONFDB_ITEM_INTEGER)
        return CONFDB_INVALID_TYPE;
